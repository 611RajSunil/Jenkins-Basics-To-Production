...
Welcome to **Day 5 Build, Push & Run a Flask App with Jenkins**! Today we’ll make Jenkins actually *ship* something. We’ll set up **Docker-outside-of-Docker (DooD)** so the Jenkins container can use the host’s Docker Engine, then build a tiny **Flask** app, **containerize** it, **push** the image to a **private Docker Hub repository**, and **deploy** it on the host for a quick smoke test. You’ll see the full CI loop end-to-end—**code → image → private registry → running container**—while keeping secrets safe with Jenkins Credentials.
...
Use **`Our GitHub Repo Account detail URL`** as the example I’m using in the lecture. **You should use your own private repo** (on GitHub, GitLab, Bitbucket—any git host is fine).
...
* **Repository URL:** `Our GitHub repo URL detail`
...
# 2) Image naming
IMAGE="docker.io/rajkdsunil/demo-app-flask"
TAG="${BUILD_NUMBER}"
...
docker rm -f demo-app-flask || true
docker run -d --name demo-app-flask -p 5000:5000 "$IMAGE:$TAG"
...
> Because the job runs **inside** your Jenkins container but talks to the **host** Docker daemon (via `/var/run/docker.sock`), the deployed container (`demo-app-flask`) will appear in your **host** `docker ps`.
